...
import {
  ArrowLeft, Bold, Italic, Heading2, List, Link2,
    ...
    Promise.all([
      ...
      .then(([catData, postData]) => {
        ...
        if (post) {
          ...
          setAuthorPhoto(post.authorPhoto || "");
          setAuthorPhotoPreview(post.authorPhoto || "");
          ...
          setSelectedCategoryIds(
            (post.categories || []).map((pc: { category: Category }) => pc.category.id)
          );
        }
      })
      .catch(console.error)
    ...
    <div>
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Update your article or news post</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="grid gap-5">
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none" required />
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-gray-700">URL Slug *</label>
              <input type="text" value={slug} onChange={(e) => setSlug(e.target.value)} className="w-full rounded-lg border border-gray-300 bg-gray-50 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none" required />
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-gray-700">Excerpt</label>
              <textarea value={excerpt} onChange={(e) => setExcerpt(e.target.value)} rows={3} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none" />
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="rounded-lg border border-gray-300 overflow-hidden focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500">
            ...
            <textarea id="blog-content" value={content} onChange={(e) => setContent(e.target.value)} rows={14} className="w-full px-3 py-3 text-sm text-gray-900 focus:outline-none resize-y font-mono" required />
          </div>
        </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  {authorPhotoPreview ? (
                    ...
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gray-100"><Upload className="h-4 w-4 text-gray-400" /></div>
                  )}
