...
// Meta Data untuk Modul-modul PMO
const PMO_MODULES_META: PMOModuleMeta[] = [
  { id: "manajemen-tugas", icon: LayoutList, keyIndex: 0, benefitsCount: 3 },
  { id: "timeline-gantt", icon: CalendarDays, keyIndex: 1, benefitsCount: 3 },
  { id: "sumber-daya", icon: Users, keyIndex: 2, benefitsCount: 3 },
  { id: "anggaran-proyek", icon: Wallet, keyIndex: 3, benefitsCount: 3 },
  { id: "kolaborasi", icon: FolderKanban, keyIndex: 4, benefitsCount: 3 },
  { id: "laporan-kinerja", icon: BarChart3, keyIndex: 5, benefitsCount: 3 },
];

// Meta Data untuk Langkah-langkah PMO
const STEPS_META: { icon: LucideIcon; keyIndex: number }[] = [
  { icon: ClipboardList, keyIndex: 0 }, // Perencanaan / Inisiasi
  { icon: Target, keyIndex: 1 },        // Eksekusi / Pelacakan
  { icon: Rocket, keyIndex: 2 },        // Pengiriman / Penyelesaian
];
...
export default function PmoPage() {
  ...
  const currentModuleMeta = PMO_MODULES_META[activeTab];
  ...
  const activeBenefits = Array.from({ length: currentModuleMeta.benefitsCount }).map((_, idx) =>
    t(`products.pmo.modules.${currentModuleMeta.keyIndex}.benefit.${idx}`)
  );
  ...
  return (
    <>
      ...
      <div className="border-b border-foreground/10 bg-background/80 backdrop-blur-md sticky top-0 z-30">
        <div className="mx-auto flex max-w-7xl items-center gap-1.5 px-4 py-3 text-[13px] text-foreground/60 sm:px-6 lg:px-8 font-medium">
          <Link href="/" className="hover:text-foreground transition-colors">
            {t("products.pmo.breadcrumb.home")}
          </Link>
          <ChevronRight className="h-3.5 w-3.5" />
          <Link href="/#products" className="hover:text-foreground transition-colors">
            {t("products.pmo.breadcrumb.products")}
          </Link>
          <ChevronRight className="h-3.5 w-3.5" />
          <span className="text-primary font-bold">PMO</span>
        </div>
      </div>
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            ...
            <div>
              <Reveal delay={100}>
                <h1 className="mt-6 font-display font-extrabold text-[42px] leading-[1.08] tracking-tight text-foreground sm:text-[56px] lg:text-[62px]">
                  {t("products.pmo.hero.title.0")} <br className="hidden sm:block" />
                  <span className="bg-linear-to-r from-blue-300 via-blue-500 to-blue-600 bg-clip-text text-transparent">
                    {t("products.pmo.hero.title.1")}
                  </span>
                </h1>
              </Reveal>
                ...
                <div className="mt-10 pt-6 border-t border-foreground/10 flex flex-wrap items-center gap-6 text-[13px] font-semibold text-foreground/60">
                  {[0, 1, 2].map((idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      ...
                      <span>{t(`products.pmo.hero.trust.${idx}`)}</span>
                    </div>
                  ))}
                </div>
              </Reveal>
            ...
            <Reveal from="right" delay={200}>
              <div className="relative mx-auto max-w-md lg:max-w-none rounded-[36px] border border-indigo-500/30 bg-linear-to-b from-indigo-500/10 to-background p-6 sm:p-10 shadow-2xl">
                {/* Top Badge Banner */}
                ...
                <div className="mt-8 space-y-5">
                  <div className="flex items-start gap-4 rounded-2xl bg-background/80 p-4 shadow-sm border border-foreground/5">
                    ...
                    <div>
                      <h4 className="font-display font-bold text-[16px] text-foreground">
                        {t("products.pmo.hero.card.tracking.title")}
                      </h4>
                      <p className="text-[13px] text-foreground/60 leading-relaxed mt-0.5">
                        {t("products.pmo.hero.card.tracking.desc")}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-4 rounded-2xl bg-background/80 p-4 shadow-sm border border-foreground/5">
                    ...
                    <div>
                      <h4 className="font-display font-bold text-[16px] text-foreground">
                        {t("products.pmo.hero.card.resource.title")}
                      </h4>
                      <p className="text-[13px] text-foreground/60 leading-relaxed mt-0.5">
                        {t("products.pmo.hero.card.resource.desc")}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8 rounded-2xl border border-primary/20 bg-primary/5 p-4 text-center">
                  <p className="text-[13.5px] font-medium text-foreground/80 italic">
                    {t("products.pmo.hero.card.quote")}
                  </p>
                </div>
              </div>
            </Reveal>
      ...
      <section className="border-y border-foreground/10 bg-foreground/5 py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 gap-8 divide-y divide-foreground/10 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            {[
              { val: 40, suf: "%", idx: 0, dec: 0 }, // e.g. Peningkatan Produktivitas
              { val: 100, suf: "%", idx: 1, dec: 0 }, // e.g. Visibilitas Real-time
              { val: 30, suf: "%", idx: 2, dec: 0 }, // e.g. Penurunan Keterlambatan
            ].map((st) => (
              <Reveal
                key={st.idx}
                delay={st.idx * 100}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-16 grid grid-cols-1 gap-8 lg:grid-cols-[1fr_1.3fr] lg:items-start">
            ...
            <Reveal className="sticky top-28">
              <Card
                variant="transparent"
                className="relative overflow-hidden rounded-[36px] border-2 border-indigo-500/20 bg-linear-to-br from-indigo-500/10 to-background p-8 sm:p-12 shadow-2xl"
              >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <span className="font-mono text-[12px] font-bold uppercase tracking-wider text-primary">
                        {t(`products.pmo.modules.${currentModuleMeta.keyIndex}.tagline`)}
                      </span>
                      <h3 className="font-display font-extrabold text-[26px] text-foreground sm:text-[32px] leading-tight mt-0.5">
                        {t(`products.pmo.modules.${currentModuleMeta.keyIndex}.title`)}
                      </h3>
                    </div>
                  </div>
                </div>
                ...
                <p className="mt-6 text-[17px] leading-relaxed text-foreground/80 font-normal">
                  {t(`products.pmo.modules.${currentModuleMeta.keyIndex}.desc`)}
                </p>
                ...
                <div className="mt-8 rounded-2xl bg-background/80 p-6 border border-foreground/10 shadow-sm">
                  <h4 className="font-display font-bold text-[15px] uppercase tracking-wider text-foreground">
                    {t("products.pmo.modules.capabilitiesHeading")}
                  </h4>
            ...
            <Reveal delay={200}>
              <div className="h-full rounded-[32px] border border-foreground/10 bg-background p-8 shadow-xl flex flex-col justify-between">
                <div>
                  ...
                  <div className="mt-6 space-y-4">
                    <div className="p-4 rounded-2xl bg-red-500/5 border border-red-500/20 text-[14px] text-foreground/80">
                      {t("products.pmo.comparison.resource.oldDesc")}
                    </div>
                    <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-[14px] font-bold text-foreground">
                      {t("products.pmo.comparison.resource.newDesc")}
                    </div>
                  </div>
                </div>
                <div className="mt-8 pt-4 border-t border-foreground/10 flex items-center gap-2 text-primary font-bold text-[14px]">
                  <Users className="h-4 w-4" /> {t("products.pmo.comparison.resource.footer")}
                </div>
              </div>
            </Reveal>
          </div>
      ...
      <section className="border-t border-foreground/10 bg-indigo-50/50 dark:bg-indigo-950/20 py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-[1fr_1.2fr] lg:gap-16">
            <Reveal>
              ...
              <div className="mt-8">
                ...
                >
                  {t("products.pmo.ecosystem.learnMore")}
                  <ArrowRight className="h-5 w-5" />
                </Link>
              </div>
            </Reveal>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              <Reveal delay={150}>
                <Card
                  variant="transparent"
                  className="flex h-full flex-col justify-between rounded-[32px] border-2 border-indigo-500/20 bg-background p-8 shadow-xl transition-transform hover:-translate-y-1"
                >
                  <div>
                    <span className="rounded-full bg-primary/10 px-3 py-1 font-mono text-[11px] font-extrabold text-primary">
                      {t("products.pmo.ecosystem.hr.badge")}
                    </span>
                    <h3 className="mt-4 font-display font-extrabold text-[24px] text-foreground">
                      {t("products.pmo.ecosystem.hr.title")}
                    </h3>
                    <p className="mt-3 text-[14.5px] leading-relaxed text-foreground/70">
                      {t("products.pmo.ecosystem.hr.desc")}
                    </p>
                  </div>
                  <Link
                  ...
                  >
                    {t("products.pmo.ecosystem.hr.link")}
                    <ArrowRight className="h-4 w-4" />
                  </Link>
                </Card>
              </Reveal>
              ...
              <Reveal delay={250}>
                ...
                >
                  <div>
                    <span className="rounded-full bg-primary/10 px-3 py-1 font-mono text-[11px] font-extrabold text-primary">
                      {t("products.pmo.ecosystem.finance.badge")}
                    </span>
                    <h3 className="mt-4 font-display font-extrabold text-[24px] text-foreground">
                      {t("products.pmo.ecosystem.finance.title")}
                    </h3>
                    <p className="mt-3 text-[14.5px] leading-relaxed text-foreground/70">
                      {t("products.pmo.ecosystem.finance.desc")}
                    </p>
                  </div>
                  <Link
                    href="/products/aiso"
                    className="mt-6 inline-flex items-center gap-1.5 text-[14px] font-bold text-primary hover:underline"
                  >
                    {t("products.pmo.ecosystem.finance.link")}
                    <ArrowRight className="h-4 w-4" />
                  </Link>
                </Card>
              </Reveal>
